...
struct rgb hsv2rgb(struct hsv color)
{
    ...
    float x = chroma * (1 - std::abs(std::fmod(hue, 2) - 1));
...
int main()
{
    ...
    for(int y = 0; y<YRES; ++y)
    {
        for(int x = 0; x<XRES; ++x)
        {
            ...
            struct rgb pixel_color = hsv2rgb({hue, saturation, color});

            pixels[y][x][0] = pixel_color.r * 255;
            pixels[y][x][1] = pixel_color.g * 255;
            pixels[y][x][2] = pixel_color.b * 255;
        }
    }
